...
import { Button, Dropdown, Input, TextArea, Card, Form, Grid, Image } from "semantic-ui-react";
...
export default function JobPostingAdd() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      values.employerId = 2;
      jobPostingService.addJobPosting(values).then((result) => alert(result.data.message));
    },
  });
  ...
  useEffect(() => {
    ...
    workingTimeService.getWorkingTimes().then((result) => setWorkTimes(result.data.data));
